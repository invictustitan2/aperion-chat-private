...
export async function exportData(options: { output?: string }) {
    ...
    try {
      ...
      if (res.ok) {
        ...
        if (Array.isArray(data)) {
            allData.push(...data);
            console.log(chalk.green(`✓ Fetched ${data.length} ${endpoint} records`));
        }
      } else {
        console.error(chalk.red(`❌ Failed to fetch ${endpoint}: ${res.statusText}`));
      }
    } catch (error: any) {
  ...
  if (options.output) {
    ...
    console.log(chalk.green(`✓ Exported ${allData.length} records to ${options.output}`));
  } else {
    console.log(jsonl);
  }
}
