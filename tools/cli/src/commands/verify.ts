...
export async function verify() {
  ...
  try {
    ...
    if (res.ok) {
      ...
      console.log(chalk.gray(`Found ${Array.isArray(data) ? data.length : 0} identity records`));
    } else {
      console.error(chalk.red(`❌ API returned ${res.status}: ${res.statusText}`));
      process.exit(1);
    }
  } catch (error: any) {
    console.error(chalk.red(`❌ Failed to connect to API: ${error.message}`));
    process.exit(1);
  }
}
