...
| **Auth**     | `VITE_AUTH_TOKEN` (in `.env`) | Cloudflare Access / Secrets     | Cloudflare Access / Secrets          |
...
| :---------------------- | :------------------------------ | :------------------------------- | :----------------------------------- |
| `API_TOKEN`             | Worker-side bearer token secret | n/a (Worker secret)              | `wrangler secret` (Worker)           |
...
| `VITE_AUTH_TOKEN`       | Client bearer token (baked in)  | `.env`                           | GitHub Actions build env             |
