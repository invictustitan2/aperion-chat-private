...
3.  **Provenance required**: All memory records must have provenance. We must know *why* a memory exists and *where* it came from.
...
5.  **Always progressive**: We do not remove features to simplify maintenance unless they are replaced by superior equivalents. We build *up*, not *down*.
